...
__inline __attribute__((always_inline)) void WaveSolver::updateWaveField(const int n) {
    ...
    for (int i = 1; i < NY-1; ++i) {
        ...
        for (int j = 1; j < NX-4; j += 4) {
            ...
            __m256d term1 = _mm256_mul_pd(px1, diff_right);
            __m256d term2 = _mm256_mul_pd(px2, diff_left);
            __m256d term3 = _mm256_mul_pd(py1, diff_up);
            __m256d term4 = _mm256_mul_pd(py2, diff_down);

            // Sum all terms and multiply by tau2
            ...
            __m256d result = _mm256_add_pd(
                _mm256_sub_pd(_mm256_mul_pd(_two, current), next),
                tau2sum
            );
