...
__inline __attribute__((always_inline)) void WaveSolver::updateWaveField(const int n) {
    ...
    // Base pointers for the current time step grid
    // double* curr_grid = &data[gridStride * currentGridIndex];
    // double* next_grid = &data[gridStride * nextGridIndex];
    ...
    for (int i = 1; i < NY-1; ++i) {
        ...
        for (int j = 1; j < NX-4; j += 4) {
            ...
            __m256d sum = _mm256_mul_pd(px1, diff_right);
            sum = _mm256_fmadd_pd(px2, diff_left, sum);
            sum = _mm256_fmadd_pd(py1, diff_up, sum);
            sum = _mm256_fmadd_pd(py2, diff_down, sum);

            __m256d tau2sum = _mm256_mul_pd(_tau2, sum);

            // 2 * current - next + tau2sum
            __m256d result = _mm256_add_pd(
                _mm256_fmsub_pd(_two, current, next),
                tau2sum
            );
