...
void GnuplotRealTime::updatePlot(double *data, const std::string &filename, bool save_png, size_t size) {
    // if (data.size() != ny || data[0].size() != nx) {
    //     throw std::runtime_error("Data size doesn't match dimensions");
    // }
    // Write binary data to temporary file
    ...
    size_t written = fwrite(data, sizeof(double), size, fp);
    if (written != size) {
        fclose(fp);
        throw std::runtime_error("Failed to write the expected amount of data to temporary file");
    }
...
void GnuplotRealTime::updatePlot(const std::vector<double>& data, const std::string& filename, bool save_png) {
    // if (data.size() != ny || data[0].size() != nx) {
    //     throw std::runtime_error("Data size doesn't match dimensions");
    // }
    ...
    if (!fp) {
        ...
        size_t written = fwrite(data.data(), sizeof(double), data.size(), fp);
